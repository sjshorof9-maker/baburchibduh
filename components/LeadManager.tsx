...
const LeadManager: React.FC<LeadManagerProps> = ({ moderators, leads, orders, onAssignLeads, onBulkUpdateLeads, onDeleteLead }) => {
  ...
  const cleanPhoneNumber = (raw: any) => {
    ...
    // Remove Bangladesh prefixes to normalize
    ...
    leads.forEach(l => {
      ...
      if (!contactMap[phone]) {
        contactMap[phone] = {
          ...
          address: l.address || '',
          leadId: l.id, 
          lastCallDate: callDate?.toISOString() || null,
  ...
  const processDataArray = (data: any[]) => {
    ...
    }).filter(l => l.phoneNumber.length >= 11);

    if (newLeads.length > 0) {
      onAssignLeads(newLeads);
      showToast(`✅ ${newLeads.length} টি লিড সফলভাবে ইমপোর্ট করা হয়েছে!`);
    } else {
      alert("❌ কোনো ভ্যালিড ফোন নম্বর পাওয়া যায়নি! কলামের নাম চেক করুন।");
    }
  ...
  const handleApplyRange = () => {
    ...
    if (isNaN(start) || isNaN(end) || start < 1 || end < start) {
      alert("⚠️ সঠিক রেঞ্জ দিন");
      return;
    }
    const phonesInRange = filteredContacts.slice(start - 1, end).map(c => c.phone);
    setStrategicSelectedPhones(prev => Array.from(new Set([...prev, ...phonesInRange])));
    showToast(`✅ S.N. ${start} থেকে ${end} সিলেক্ট করা হয়েছে!`);
  };

  const handleQuickSelect = (count: number) => {
    const firstUnselected = filteredContacts.findIndex(c => !strategicSelectedPhones.includes(c.phone));
    if (firstUnselected === -1) return;
    const endIdx = Math.min(firstUnselected + count, filteredContacts.length);
    const slice = filteredContacts.slice(firstUnselected, endIdx).map(c => c.phone);
    setStrategicSelectedPhones(prev => Array.from(new Set([...prev, ...slice])));
  };
    ...
    <div className="space-y-8 pb-32 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] mt-1">Strategic Selection Hub</p>
        </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-slate-950 p-8 rounded-[3rem] text-white shadow-2xl border border-white/5 relative overflow-hidden">
            ...
            <div className="space-y-5">
               ...
               <div className="bg-white/5 p-5 rounded-2xl border border-dashed border-white/10 space-y-4">
                 <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Select By S.N. Range</p>
                 <div className="flex gap-2">
                    <input type="number" placeholder="From" value={rangeStart} onChange={(e) => setRangeStart(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-[10px] font-black outline-none"/>
                    <input type="number" placeholder="To" value={rangeEnd} onChange={(e) => setRangeEnd(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-[10px] font-black outline-none"/>
                 </div>
                 <button onClick={handleApplyRange} className="w-full py-2 bg-slate-800 text-white rounded-lg font-black text-[9px] uppercase transition-all">Apply Range</button>
                 <div className="flex gap-2 pt-2 border-t border-white/5">
                    <button onClick={() => handleQuickSelect(50)} className="flex-1 py-2 bg-indigo-900/30 text-indigo-300 rounded-lg text-[8px] font-black uppercase hover:bg-indigo-900/50 transition-all border border-indigo-900/40">+50</button>
                    <button onClick={() => handleQuickSelect(100)} className="flex-1 py-2 bg-indigo-900/30 text-indigo-300 rounded-lg text-[8px] font-black uppercase hover:bg-indigo-900/50 transition-all border border-indigo-900/40">+100</button>
                 </div>
               </div>
          ...
          <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100">
            <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6 border-b border-slate-50 pb-4">Age Thresholds</h3>
            <div className="space-y-4">
               <div className="space-y-1">
                 <label className="text-[8px] font-black text-slate-400 uppercase">Min Days Since Last Call</label>
                 <input type="number" value={minDaysSinceCall} onChange={(e) => setMinDaysSinceCall(e.target.value)} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-[11px] font-black outline-none" placeholder="e.g. 7" />
               </div>
               <div className="space-y-1">
                 <label className="text-[8px] font-black text-slate-400 uppercase">Min Days Since Last Order</label>
                 <input type="number" value={minDaysSinceOrder} onChange={(e) => setMinDaysSinceOrder(e.target.value)} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-[11px] font-black outline-none" placeholder="e.g. 30" />
               </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-3 space-y-6">
           <div className="bg-white p-6 rounded-[3rem] shadow-sm border border-slate-100 flex flex-col md:flex-row gap-4 items-center">
              <div className="relative flex-1 group">
                ...
                <input type="text" placeholder="Search Identity (Name or Phone)..." value={searchPhone} onChange={(e) => setSearchPhone(e.target.value)} className="w-full pl-14 pr-8 py-5 bg-slate-50 border border-slate-200 rounded-[2.5rem] text-[11px] font-black outline-none focus:ring-4 focus:ring-indigo-500/5 transition-all"/>
              </div>
           ...
           <div className="bg-white rounded-[3.5rem] shadow-2xl border border-slate-100 overflow-hidden">
              <div className="bg-slate-50 px-10 py-5 border-b border-slate-100 flex justify-between items-center">
                 <div className="flex items-center gap-3">
                   <input type="checkbox" checked={strategicSelectedPhones.length > 0 && strategicSelectedPhones.length === filteredContacts.length} onChange={() => { if (strategicSelectedPhones.length === filteredContacts.length) setStrategicSelectedPhones([]); else setStrategicSelectedPhones(filteredContacts.map(c => c.phone)); }} className="w-5 h-5 rounded-lg border-slate-300 text-indigo-600 focus:ring-indigo-500" />
                   <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Master Control: {filteredContacts.length} Record(s)</span>
                 </div>
              </div>
                ...
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-slate-50">
                      ...
                      <tr key={contact.phone} className={`group hover:bg-slate-50/50 transition-colors ${strategicSelectedPhones.includes(contact.phone) ? 'bg-indigo-50/30' : ''}`}>
                        ...
                        <td className="px-10 py-8">
                           <div className="flex items-center gap-4">
                              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-base shadow-sm ${contact.totalOrders > 0 ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-400'}`}>
                    ...
                    {filteredContacts.length === 0 && (
                      <tr><td colSpan={5} className="py-40 text-center text-slate-200 font-black uppercase text-[12px] tracking-widest italic opacity-30">No Records Found In Current Protocol</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
