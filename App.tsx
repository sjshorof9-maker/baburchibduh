...
const App: React.FC = () => {
  ...
  // Heartbeat Effect: Updates "last_seen" every minute if a moderator is logged in
  useEffect(() => {
    ...
    const interval = setInterval(updatePresence, 60000); // Every 60 seconds
  ...
  useEffect(() => {
    const initApp = async () => {
      try {
        ...
        if (dbModerators) setModerators(dbModerators.map(m => ({
          ...
          lastSeen: m.last_seen // Added tracking field
        })));
    ...
    // Set up polling for admin to see moderator activity updates
    const pollInterval = setInterval(async () => {
        const { data } = await supabase.from('moderators').select('*');
        if (data) setModerators(data.map(m => ({
          id: m.id,
          name: m.name,
          email: m.email,
          role: m.role as UserRole,
  ...
  const handleCreateOrder = async (newOrder: Order) => {
    try {
      ...
      console.error("handleCreateOrder Catch Block:", err);
      throw err;
    }
  };
  ...
  return (
    <Layout user={currentUser} onLogout={handleLogout} activeTab={activeTab} setActiveTab={setActiveTab} logoUrl={logoUrl}>
      <div className="animate-in fade-in duration-500">
        {activeTab === 'dashboard' && <Dashboard orders={orders} products={products} leads={leads} currentUser={currentUser} />}
        ...
        {activeTab === 'moderators' && currentUser.role === UserRole.ADMIN && <ModeratorManager moderators={moderators} leads={leads} onAddModerator={handleAddModerator} onDeleteModerator={handleDeleteModerator} />}
